...
my ($in_file, $filetype, $fwd_primer, $rev_primer, $min_readlen, $max_readlen, $silent) = get_args_and_error_check();
	...
	if($filetype eq "fasta"){
		...
		if($sequence =~ /^[ACGT]{0,$mbases}$fwd_primer_trimmed([ACGT]+)$rev_primer_trimmed[ACGT]{0,$mbases}$/){
			my $trimmed_sequence = $1;
                        my $seq = "$fwd_primer$trimmed_sequence$rev_primer";
                        my $seqlen = length $seq;
                        if( ($seqlen>=$min_readlen) && ($seqlen<=$max_readlen)){
			   print ">$header\n";
			   print "$seq\n";
                        }
		}
		if($sequence =~ /^[ACGT]{0,$mbases}$rev_primerRC_trimmed([ACGT]+)$fwd_primerRC_trimmed([ACGT]{0,$mbases})$/){
			my $trimmed_sequence = $1;
			my $seq = "$rev_primerRC$trimmed_sequence$fwd_primerRC\n";
                        my $seqlen = length $seq;
                        if( ($seqlen>=$min_readlen) && ($seqlen<=$max_readlen)){
                           print ">$header\n";
                           print "$seq\n";
	...
	if($filetype eq "fastq"){
		...
		if($sequence =~ /^[ACGT]{0,$mbases}$fwd_primer_trimmed([ACGT]+)$rev_primer_trimmed[ACGT]{0,$mbases}$/){
			...
			my $dummy_qual_end = $last_qual x $rev_primer_len;
                        my $seq = "$fwd_primer$trimmed_sequence$rev_primer";
                        my $qual = "$dummy_qual_start$qual_substr$dummy_qual_end";
			my $seqlen = length $seq;
                        if( ($seqlen>=$min_readlen) && ($seqlen<=$max_readlen)){
                        print "\@$header\n";
			print "$seq\n";
...
sub get_args_and_error_check{
	...
	my ($ccs_file, $filetype, $fwd_primer, $rev_primer, $min_readlen, $max_readlen, $silent);
    ...
    unless(defined($min_readlen)) {
                $min_readlen = 1;
        }

    unless(defined($max_readlen)) {
                $max_readlen = 1;
        }
	...
	if(defined $error_to_print) {
		...
		return ($ccs_file, $filetype, $fwd_primer, $rev_primer, $min_readlen, $max_readlen, $silent);
	}
}
...
./filter_primers.pl [options] B<--help> B<-f> B<-t> B<-fwd> B<-rev> B<-minl> B<-maxl> B<--silent>
...
Minimum read length (default: 1)

=item B<-maxl>

Maximum read length (default: 1000000)
